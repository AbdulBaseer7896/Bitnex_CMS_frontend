  ...
  return (
    <header className="sticky top-0 z-30 glass border-b border-[#4BBFBF]/10 px-6 py-4 flex items-center justify-between gap-4">
      <div className="pl-10 lg:pl-0">
        ...
        <p className="text-slate-500 text-xs mt-0.5">{greeting()}, {user?.first_name || user?.username} 👋</p>
      </div>
      <div className="flex items-center gap-3">
        <div className="relative hidden md:block">
          <HiOutlineSearch className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500 w-4 h-4"/>
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search..."
            className="glass rounded-xl pl-9 pr-4 py-2 text-sm text-slate-300 placeholder-slate-600 focus:outline-none w-48 lg:w-60"
